...
A constructor $\kappa$ is a string that denotes
...
a condition $\cond$ denotes a prose expression that is evaluated to a boolean value.
...
{$(\ssf{LABEL\_} \; n \; i'^*\; t\; (\ssf{BR}\; t')\; i^*$)} with two fresh variables $t$ and $t'$.
...
the same left-hand sides. The definitions of $t$ and $t'$ are available as premises:
