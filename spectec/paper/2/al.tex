...
\inred{
If the reordering fails for every possible partition, this means that
the original intention was indeed having a cyclic bindings, which SpecTec cannot handle yet.
One of the auxiliary helper function in the official specification contained this
cyclic definition, and we re-defined that function in a way that avoids cyclic binding.
Except for that, the preprocessing succeed for all the Wasm semantic definitions.
}
